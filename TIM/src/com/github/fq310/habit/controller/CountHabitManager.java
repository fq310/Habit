...
public class CountHabitManager {
	...
	private CountHabit readHabitFrom(Cursor cursor) {
		...
		String latestCheckTime = cursor.getString(
				cursor.getColumnIndexOrThrow(CountHabitEntry.COLUMN_NAME_LATEST_CHECK_TIME));
		HabitDate currentDate = new HabitDate();
		if (!currentDate.toString().equals(latestCheckTime)) todayChecked = 0;
	...
	private ContentValues getUpdateValues(CountHabit project) {
		...
		values.put(CountHabitEntry.COLUMN_NAME_LATEST_CHECK_TIME, new HabitDate().toString());
		return values;
	}
